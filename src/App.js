...
export default function App() {
  ...
  const renderView = () => {
    switch (view) {
      case 'dashboard': return <Dashboard cases={cases} categories={categories} users={DEMO_USERS} onSelectCase={c => { setSelCase(c); setView('cases'); }} onNavigate={goTo} isMobile={isMobile} />;
      case 'cases': return <Cases cases={cases} users={DEMO_USERS} sops={sops} categories={categories} curUser={curUser} selCase={selCase} setSelCase={setSelCase} onCreateCase={createCase} onUpdateCase={updateCase} onAddComment={addComment} onDeleteCase={id => { setCases(p => p.filter(c => c.id !== id)); setSelCase(null); }} onOpenAI={openAI} onViewSOP={s => { setSelSOP(s); goTo('sops'); }} isMobile={isMobile} />;
      case 'sops': return <SOPs sops={sops} users={DEMO_USERS} categories={categories} curUser={curUser} selSOP={selSOP} setSelSOP={setSelSOP} onUpdateSOP={updateSOP} onCreateSOP={createSOP} isMobile={isMobile} />;
      case 'wiki': return <Wiki wiki={wiki} users={DEMO_USERS} curUser={curUser} selWiki={selWiki} setSelWiki={setSelWiki} onUpdateWiki={updateWiki} onCreateWiki={createWiki} onOpenAI={openAI} isMobile={isMobile} />;
      case 'recurring': return <Recurring recurring={recurring} users={DEMO_USERS} categories={categories} curUser={curUser} canEdit={canEdit} onUpdate={updateRecurring} onCreate={createRecurring} isMobile={isMobile} />;
      case 'team': return <Team users={DEMO_USERS} cases={cases} curUser={curUser} isMobile={isMobile} />;
      case 'settings': return <Settings users={DEMO_USERS} categories={categories} curUser={curUser} setCurUser={setCurUser} setCategories={setCategories} isMobile={isMobile} />;
      default: return null;
    }
  };
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      <Sidebar view={view} curUser={curUser} sideOpen={sideOpen} setSideOpen={setSideOpen} goTo={goTo} onOpenAI={() => openAI(null)} isMobile={isMobile} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <Topbar notifs={notifs} setNotifs={setNotifs} showNotifs={showNotifs} setShowNotifs={setShowNotifs} setSideOpen={setSideOpen} cases={cases} onSelectCase={c => { setSelCase(c); setView('cases'); }} onNavigate={goTo} isMobile={isMobile} />
        <main style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '14px' : '22px', maxWidth: '1100px', margin: '0 auto', width: '100%' }}>
          {renderView()}
        </main>
      </div>
      <AIPanel isOpen={showAI} onClose={() => { setShowAI(false); setAiContext(null); }} sops={sops} wiki={wiki} contextCase={aiContext} isMobile={isMobile} />
    </div>
  );
}
