...
// ═══════════ RECURRING TASKS ═══════════
export function Recurring({ recurring, users, categories, curUser, canEdit, onUpdate, onCreate, isMobile }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
        <h1 style={{ fontSize: isMobile ? '20px' : '22px', fontWeight: 800, margin: 0 }}>Recurring Tasks</h1>
        ...
        recurring.map(r => { const a = getUser(r.assignTo, users); const p = getPriority(r.priority); return (
          <Card key={r.id} style={{ marginBottom: '8px' }}>
            <div style={{ padding: isMobile ? '12px' : '14px 16px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: T.rad, backgroundColor: r.active ? T.priBg : T.surfAlt, color: r.active ? T.pri : T.textMut, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', flexShrink: 0 }}>🔄</div>
              ...
              {!isMobile && a && <Avatar user={a} size="sm" />}
...
// ═══════════ TEAM ═══════════
export function Team({ users, cases, curUser, isMobile }) {
  return (
    <div>
      <h1 style={{ fontSize: isMobile ? '20px' : '22px', fontWeight: 800, margin: '0 0 16px' }}>Team</h1>
          ...
          return (
            <Card key={u.id}><div style={{ padding: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: '12px', color: T.textMut, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{u.email}</div>
...
// ═══════════ SETTINGS ═══════════
export function Settings({ users, categories, curUser, setCurUser, setCategories, isMobile }) {
  const [newCat, setNewCat] = useState('');
  const isAdmin = curUser.role === 'admin';

  return (
    <div>
      <h1 style={{ fontSize: isMobile ? '20px' : '22px', fontWeight: 800, margin: '0 0 20px' }}>Settings</h1>

      <Card style={{ marginBottom: '14px' }}><div style={{ padding: isMobile ? '14px' : '18px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
          {users.map(u => <Btn key={u.id} s="sm" v={curUser.id === u.id ? 'primary' : 'secondary'} onClick={() => setCurUser(u)}>{isMobile ? u.name.split(' ')[0] : u.name} ({u.role})</Btn>)}
        </div>
      </div></Card>
      ...
      <Card><div style={{ padding: isMobile ? '14px' : '18px' }}>
        ...
        {categories.map((c, i) => (
          <div key={c} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 10px', backgroundColor: T.surfAlt, borderRadius: T.radSm, marginBottom: '4px' }}>
            <span style={{ fontSize: '13px' }}>{c}</span>
            {isAdmin && categories.length > 1 && <button onClick={() => setCategories(p => p.filter((_,idx)=>idx!==i))} style={{ background:'none', border:'none', cursor:'pointer', color:T.textMut, fontSize:'18px', padding: '4px' }}>×</button>}
          </div>
        ))}
